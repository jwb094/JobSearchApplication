...
export const getJobs = function (page, limit, data, sort, filters) {
  ...
  sortedData = sortedData.filter((job) => {
    ...
    const matchesSalaries =
      ...
      filters.salaries.some((salary) => Number(job.salary_min.replace(",","")) >= salary);
    ...
    return matchesJobType && matchesLocationType && matchesExperienceType && matchesSalaries;
  });
...
export const getJobDescByID = (job_id) => {
  return jobs.find((job) => job.id === Number(job_id));
};
